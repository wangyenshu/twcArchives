...
''2006.03.30 [2.9.1]''
when extracting store area from remote URL, look for "</body>" instead of "</body>\n</html>" so it will match even if the "\n" is absent from the source.
''2006.03.30 [2.9.0]''
...
#importPanel .chk { width:auto; margin:1px; }\
...
function copyTiddlersToStore(src,tiddlers,filter,quiet,ask)
{
	...
	if (tiddlers) for (var t=0;t<tiddlers.length;t++) {
		...
		if (ask && !confirm((theExisting?"Update":"Add")+" tiddler '"+theInbound.title+"'\nfrom "+src))
