...
var admin = config.macros.TiddlySpaceAdmin = {
	locale: {
		username: "brugernavn",
		success: "Lige nu er du logget ind som %0.",
		loginError: "der opstod en fejl ved indlogning i %0: %1",
		forbiddenError: "log ind mislykkedes for <em>%0</em>: brugernavn og password passer ikke sammen",
		password: "password",
		confirmPassword: "bekræft password:",
		openid: "openid brugernavn:",
		logout: { submit: "Log ud" },
		login: { submit: "Log ind" },
		register: { submit: "Tilmeld dig" },
		identities: { submit: "Tilføj identitet" },
		userSuccess: "oprettede brugeren %0",
		userError: "brugeren <em>%0</em> findes allerede",
		spaceSuccess: "oprettede spacet %0",
		spaceError: "spacet <em>%0</em> findes allerede",
		charError: "fejl: ugyldigt brugernavn - det må kun indeholde små bogstaver, tal eller bindestreg",
		passwordError: "fejl: passwords stemmer ikke overens",
		listError: "Der opstod en fejl ved hentning af identiteter for brugeren %0"
	},
	elements: {
		openid: function() {
			return { name: "openid", placeholder: "openid" };
		},
		password: function(repeated) {
			...
			var placeholder = repeated ? "password (igen)" : "password";
			return { type: "password", name: name, placeholder: placeholder };
		},
		username: function() {
			return { name: "username", placeholder: "username" };
		},
		redirect: function() {
...
config.paramifiers.auth = {
	locale: {
		success: "det lykkedes at tilføje identiteten %0",
		error: "det lykkedes IKKE at tilføje identiten %0: %1"
	},
