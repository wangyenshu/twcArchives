...
function addCreatedsToPopup(p,when,format)
{
	...
	if (createds) {
		createds.sort();
		var e=createTiddlyElement(p,'div',null,null,'oprettet ('+createds.length+')');
...
function addModifiedsToPopup(p,when,format)
{
	...
	if (mods) {
		...
		mods.sort();
		var e=createTiddlyElement(p,'div',null,null,'ændret ('+mods.length+')');
		for(var t=0; t<mods.length; t++) {
...
function addTaggedToPopup(p,when,format)
{
	...
	if (tagged.length) var e=createTiddlyElement(p,'div',null,null,'tagget ('+tagged.length+')');
...
function addRemindersToPopup(p,when,format)
{
	...
	createTiddlyElement(p,'div',null,null,'påmindelser ('+(reminders.length||'none')+')');
	for(var t=0; t<reminders.length; t++) {
		link = createTiddlyLink(createTiddlyElement(p,'li'),reminders[t].tiddler,false);
		var diff=reminders[t].diff;
		diff=(diff<1)?'Idag':((diff==1)?'Imorgen':diff+' dage');
		var txt=(reminders[t].params['title'])?reminders[t].params['title']:reminders[t].tiddler;
		link.appendChild(document.createTextNode(indent+diff+' - '+txt));
	}
	if (readOnly) return;	// readonly... omit 'new reminder...' command
	var rem='\\<\\<reminder day:%0 month:%1 year:%2 title:"Indskriv en påmindelsestitel her"\\>\\>';
	rem=rem.format([when.getDate(),when.getMonth()+1,when.getYear()+1900]);
	var cmd="<<newTiddler label:[["+indent+"ny påmindelse...]] prompt:[[tilføj en påmindelse til '%0']]"
